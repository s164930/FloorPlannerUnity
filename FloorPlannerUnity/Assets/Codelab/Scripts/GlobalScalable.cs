...
/// Typically this is only enabled for one object per scene.
/// </para>
/// </remarks>
        ...
        if (adjustScale)
        {
            ...
            if (Input.touchCount == 1)
            {
                ...
                if (touch.deltaPosition.y > 0)
                {
                    ...
                    transform.localPosition = Vector3.MoveTowards(transform.localPosition, transform.localPosition + Vector3.up, 5 * Time.deltaTime);
                }
                else if (touch.deltaPosition.y < 0)
                {
                    Debug.Log("Direction is smaller than zero, it should go down");
                    transform.localPosition = Vector3.MoveTowards(transform.localPosition, transform.localPosition + Vector3.down, 5 * Time.deltaTime);
                }
            }
        }
    }
